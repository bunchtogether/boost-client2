...
export default ((parameters = {
  ...
  class NewComponent extends React.Component {
    constructor(props) {
      ...
      _defineProperty(this, "eventHandlerFunctions", void 0);
    ...
    async fetchValues(machines, names, delta, end = Date.now()) {
      // eslint-disable-line
      try {
        const timeseriesData = await agent.get(`/timeseries/${delta}/${end}`).query({
    ...
    subscribeToValueUpdates(machine, name) {
      ...
      const handler = data => {
        ...
        if (list && data.value && this.mounted) {
          ...
          this.setState({
            values: this.state.values.setIn([machine, name], newValue)
          });
        }
      };
